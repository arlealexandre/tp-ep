...
plot_response_times(1, Service, 1/mu*k);

```
The 2 different plot shows the average response time for the different scheduling disciplines for the G/G/k/FCFS queue and the G/G/1/FCFS queue with a server that operates at speed k. We can see that the average response time for the G/G/k/FCFS queue is always lower than the average response time for the G/G/1/FCFS queue with a server that operates at speed k. This is because in the G/G/k/FCFS queue can treat jobs in parallel while in the G/G/1/FCFS queue with a server that operates at speed k can only treat one job at a time. The parallelism in the G/G/k/FCFS queue allows to reduce the average response time.
